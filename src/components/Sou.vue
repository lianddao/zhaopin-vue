  ...
  <div class="container">
    ...
    <b-modal id="modal-center" centered size="lg" title="选择地区" hide-footer>
      ...
      <ul class="list-inline row">
        <!--有城市-->
        <li v-for="i in sheng" v-if="citysDialog(i.id).length > 0" :id="'x_'+i.id" class="col-6 col-md-4">
          {{ i.name }}
    ...
    <pre>{{ job_type_selected }}</pre>

    <hr />

    <h1>热门职位</h1>
    <ul class="list-inline row">
      <li class="col-xs-6 col-sm-6 col-md-3" v-for="i in hot">{{ i.name }}</li>
    </ul>
  ...
  export default {
    data() {
      return {
        ...
        selected: [],
        citysDialog: function (id, arg) {  // ①在 data 中定义一个函数资料citysDialog,此资料可以使用 import 中包含的对象的方法.(在methods,computed中都使用不符合格式)
    ...
    components: {
      DataSource, CityPop, Result, JobType
    },
    computed: {
      getDiDian: {
        get: function () {
    ...
    methods: {
      ...
      onSubmit() {
        ...
        this.url = 'http://sou.zhaopin.com/jobs/searchresult.ashx?' + $.param(params)
      },
