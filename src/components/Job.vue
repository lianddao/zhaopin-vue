  ...
  <div v-if="show">
    ...
    <ul class="pagination">
      <li v-for="p in pages">
        ...
        <a v-else @click="loadPage(p.index)">{{ p.text }}</a>
      </li>
    </ul>
    ...
    methods: {
      loadPage: function (n) {
        if (this.queryUrl != undefined && this.queryUrl.length > 0) {
          ...
          var href = 'http://sou.zhaopin.com/jobs/searchresult.ashx?sj=45&jl=489&sg=6d0b192c710d46a2a39a9f139c5f805d&p=' + n
          console.log('不加载默认数据')
          this.show = false
          return
        }
        ...
        $.get(href, (html) => {
          ...
          //② 取得所有行数据，加入rows中
          var rows = []
          dom.find('#newlist_list_content_table .newlist:not(:first)').each(function () {

            //① 取得1行数据，加入cols中
            var cols = []
            $(this).find("td").not(".gxsj, .fk_lv").each(function (i) {
              var j = '', v = $.trim($(this).text())
              switch (i) {
                case 0:
                  j = '"名称":"' + v + '"'
                  j += ',"链接":"' + $(this).find('a:first').attr('href') + '"'
                  break;
                case 1:
                  j = '"地址":"' + v + '"'
                  break;
          ...
          //③ 取分页值
          self.pages.length = 0
          dom.find(".pagesDown a:not('.nopress, .nopress2')").each(function (i) { //过滤掉第一页和最后一页时的'上一页'和'下一页'按钮
            var item = $(this)
            var index, disabled = false
            if (item.is('.current')) {
              disabled = true
            }
            ...
            self.pages.push({ index: index, text: item.text(), href: item.attr('href'), disabled: disabled })
          })
          //③
        })
      },
  ...
  function replaceUnknown(json) {
    ...
    try {
      ...
      if (n == 0) {
        ...
        if (temp.charAt(arr[7] - 1) === '"') {                                  //处理双引号非成对出现,格式例如 Unexpected token 字 in JSON at position 999
          var i = arr[7]
